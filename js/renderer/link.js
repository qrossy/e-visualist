...
Link.prototype.drawSLink = function()
{
	...
	if (this.space == 0){
		...
		if (this.getMainPath().attr("d") && segments.length > 4){
			...
			var end = Link.getIntersection(this.target,this.source.getCenter());
...
Link.boxIntersect = function (bbox, line){
	...
	var s = [{x:bbox.x, y:bbox.y+bbox.height}, {x:bbox.x+bbox.width, y:bbox.y+bbox.height}];
	var e = [{x:bbox.x, y:bbox.y}, {x:bbox.x, y:bbox.y+bbox.height}];
	var w = [{x:bbox.x+bbox.width, y:bbox.y}, {x:bbox.x+bbox.width, y:bbox.y+bbox.height}];
	var lines = [n, s, e, w];
	var inter = []
	for (var i in lines){
		var p1 = new Object();
...
Link.vector = function(from, to)
{
	...
	v.norm = function(){ return Math.sqrt(this.x*this.x + this.y*this.y);};
