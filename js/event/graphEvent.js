...
graphEvent.prototype.down = function(event) {
  ...
  if (Interface.modifiedEntity) {
    ...
    this.g.ctrl.addAction(Action.change, {
      e: entity,
      data: data
    });
    this.g.ctrl.run();
  ...
  if (Interface.modifiedLabel) {
    ...
    this.g.ctrl.addAction(Action.changeLabel, {
      l: Interface.modifiedLabel,
      txt: html
    });
    this.g.ctrl.run();
...
graphEvent.prototype.onZoom = function(event) {
  ...
  if (z <= 0.3 && !this.hideLabels) {
    g.svg.selectAll('foreignObject').each(function() {
      $(this).attr("visibility", "hidden");
    });
    if (this.g.gridVisible) {
      g.svg.selectAll('.grid').each(function() {
        $(this).attr("visibility", "hidden");
      });
    }
    g.svg.selectAll('path').each(function() {
      $(this).attr("shape-rendering", "crispEdges");
    });
    g.svg.selectAll('image').each(function() {
      $(this).attr("image-rendering", "optimize-speed");
    });
    ...
    g.svg.selectAll('path').each(function() {
      $(this).attr("shape-rendering", "geometric-precision");
    });
    g.svg.selectAll('image').each(function() {
      $(this).attr("image-rendering", "optimize-quality");
    });
    this.hideLabels = false;
  }
